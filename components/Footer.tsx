...
function MobileFooter() {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: "0.5px", backgroundColor: INK, opacity: 0.4, zIndex: 20 }} />
      ...
      <div style={{ flex: 1, position: "relative", zIndex: 20, marginLeft: 1 }}>
        ...
        <div style={{ padding: "14px 12px 16px", borderBottom: hRule }}>
          ...
          <p className="font-mono-frag" style={{ fontSize: 12, lineHeight: 1.5, color: INK, margin: 0 }}>
        ...
        <div style={{ padding: "14px 12px 20px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            {DEALERS.map((d) => (
              <div key={d.num} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img
                  src={d.photo}
                  alt={d.name}
                ...
                <div style={{ flex: 1 }}>
                  <div className="font-mono-frag" style={{ fontSize: 12, color: INK, lineHeight: 1.6, marginBottom: 4 }}>
                    DEALER {d.num}:<br />{d.name}<br />({d.role})
                  </div>
                  <p className="font-mono-frag" style={{ fontSize: 12, color: INK, lineHeight: 1.4, margin: "0 0 6px" }}>
                    {d.bio}
                  </p>
                  <div style={{ display: "flex", flexDirection: "column", gap: 1 }}>
                    {d.links.map((link) => (
                      <span key={link} className="font-mono-frag" style={{ fontSize: 11, color: INK, lineHeight: 1.8 }}>
                        {link}
                      </span>
                    ))}
                  </div>
    ...
    >
      ...
      >
        ...
        <img
          ...
          alt="抹茶カルテル"
          style={{ width: "42%", height: "auto", display: "block", filter: "brightness(0)", marginTop: 6, opacity: 0.85 }}
        />
      </div>
      ...
      >
        ...
        <p
          className="font-mono-frag"
          style={{ fontSize: 14, lineHeight: 1.2, fontWeight: 200, color: INK, margin: "0 0 16px 0" }}
        >
          Matcha Cartel began as a shared interest in matcha and an observation of its rapid rise as a global trend. As demand grew, scarcity followed, shifting matcha from ritual ingredient to high-value commodity.
        </p>
        <p
          className="font-mono-frag"
          style={{ fontSize: 14, lineHeight: 1.2, fontWeight: 200, color: INK, margin: 0 }}
        >
          This project reframes matcha through a darker visual language, presenting it as something controlled, traded, and desired rather than purely ceremonial. Familiar wellness aesthetics are replaced with systems of restriction, excess, and spectacle.
        </p>
      </div>
